...
**Cost note.** `sine` and `triangle` are generated by an oscillator, so they are
free at any rate and run for as long as the note does. `square`, `sawtooth` and
`random` are drawn with scheduled automation, capped at 1000 events per note-on
— at least 16 seconds of motion even at the maximum 20 Hz, after which they hold
their last value. Prefer `sine` or `triangle` for anything a note might sustain
on for a long time.
